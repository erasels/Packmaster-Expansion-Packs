...
import thePackmaster.cards.pickthemallpack.AbstractPickThemAllCard;
...
public class ResetDescriptionForCombatPatch {
    ...
    public static void resetDescriptionForCombat(CardGroup __instance) {
        for (AbstractCard c : __instance.group) {
            if (c instanceof AbstractPickThemAllCard) {
                ((AbstractPickThemAllCard)c).resetDescriptionForCombat();
            }
        }
    }
}
